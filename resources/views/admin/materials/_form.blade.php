...
<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <select name="course_id" class="form-select @error('course_id') is-invalid @enderror">
            ...
            @foreach($courses as $course)
                <option value="{{ $course->id }}" @selected(old('course_id', $material->course_id ?? '') == $course->id)>{{ $course->subject->name }} - {{ $course->classRoom->name }} ({{ $course->teacher->user->name }})</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        <label class="form-label">Upload File (PDF/Word/PPT/Video/Gambar)</label>
    ...
    <div class="col-md-6 mb-3">
        <label class="form-label">URL Video/Tautan (jika tipe link)</label>
        <input type="url" name="video_url" value="{{ old('video_url', $material->video_url ?? '') }}" class="form-control @error('video_url') is-invalid @enderror" placeholder="https://youtube.com/...">
