    ...
    <div class="col-md-6 mb-3">
        ...
        <select name="course_id" class="form-select @error('course_id') is-invalid @enderror">
            ...
            @foreach($courses as $course)
                <option value="{{ $course->id }}" @selected(old('course_id', $assignment->course_id ?? '') == $course->id)>{{ $course->subject->name }} - {{ $course->classRoom->name }} ({{ $course->teacher->user->name }})</option>
            @endforeach
        </select>
